...
class PaymentTransaction(models.Model):
    ...
    def _get_specific_rendering_values(self, processing_values):
        ...
        _logger.info(
            "Sending '/button/Prepare' request for link creation:\n%s",
    ...
    def _process_notification_data(self, notification_data):
        ...
        payment_id = notification_data.get("clientTransactionId")
        if not payment_id:
            raise ValidationError(
                _("Payphone: Received data with missing clientTransactionId.")
            )
        ...
        verified_payment_data.get("payment_type_id", "")
        # Update the payment state.
        payment_status = verified_payment_data.get("statusCode")
        if not payment_status:
            raise ValidationError(_("Payphone: Received data with missing status."))

        if payment_status in ["03", 3]:
            self._set_done()
        elif payment_status in [2, "2"]:
            self._set_canceled()
